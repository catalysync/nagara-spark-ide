...
class WorkbookExecutor:
    ...
    def execute_node(self, workbook: Any, node_id: str, preview: bool = False) -> dict:
        ...
        try:
            if node.type == NodeType.DATASET:
                ...
                with redirect_stdout(stdout_buf), redirect_stderr(stderr_buf):
                    ...
                    if node.code.strip():
                        # Try eval on last line to capture expression results
                        ...
                        if isinstance(val, DataFrame):
                            namespace["_result_"] = val
                        elif val is None:
                            pass  # will search namespace below
                # Look for a DataFrame in namespace
                df = namespace.get("_result_") or self._find_dataframe_result(namespace, node.code)
                if df is not None:
    ...
    def invalidate_node(self, node_id: str, workbook: Any):
    ...
    def _get_execution_order(self, workbook: Any, target_id: str) -> list[str]:
    ...
    def _get_upstream_dfs(self, workbook: Any, node_id: str) -> dict[str, DataFrame]:
    ...
    def _find_node(self, workbook: Any, node_id: str):
        """Find a node by ID."""
